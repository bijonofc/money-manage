<template>
  <div  class="tag-input form-control" :class="{ 'with-count': showCount, duplicate: noMatchingTag }">
    <input
      v-model="newTag"
      type="text"
      :list="id"
    ...
    <datalist  @keydown.prevent.enter="addTag(newTag)" v-if="options" :id="id">
...
export default {
  ...
  mounted() {
    this.$eventBus.$on('updateTag',this.hasTag);
  },
  methods:{
    hasTag(data)
    {
...
ul {
  --tagBgColor: var(--ab-main-color);
  --tagTextColor: var(--ab-body-color);
  list-style: none;
  display: flex;
  align-items: center;
  gap: 7px;
