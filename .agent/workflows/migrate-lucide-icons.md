...
1. Check whether the project still references the old package:
   ```bash
   grep -rl "lucide-vue-next" --include="*.{vue,js,ts,jsx,tsx}" src
   ```
...
3. Replace the import source everywhere it's used (only the string `lucide-vue-next` changes — named imports stay identical):
   ```bash
   grep -rl "lucide-vue-next" --include="*.{vue,js,ts,jsx,tsx}" src | xargs sed -i 's/lucide-vue-next/@lucide\/vue/g'
   ```
...
5. Start the dev server once and confirm icons still render — the rename is behaviorally a no-op, but verify before committing.
...
- If the project re-exports `lucide-vue-next` icons as part of its own public API, call that out separately — this migration only covers internal imports, not downstream consumers of this project.
